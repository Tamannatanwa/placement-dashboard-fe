"use client";

import { useState } from "react";
import { useGoogleLogin } from "@react-oauth/google";
import { Button } from "@/components/ui/button";
import { authApi } from "@/lib/api/auth";
import { toast } from "sonner";
import { useRouter } from "next/navigation";
import { getDashboardRoute } from "@/lib/utils/auth";

interface GoogleLoginButtonProps {
  isSignup?: boolean;
}

export function GoogleLoginButton({ isSignup = false }: GoogleLoginButtonProps) {
  const [isLoading, setIsLoading] = useState(false);
  const router = useRouter();

  const handleGoogleSuccess = async (codeResponse: any) => {
    setIsLoading(true);
    try {
      // Send the authorization code to backend
      // Backend will exchange it for user info and verify email domain
      const response = await authApi.googleLogin({
        idToken: codeResponse.code, // Authorization code
        role: "student",
      });

      if (response.access_token) {
        toast.success(isSignup ? "Account created successfully!" : "Login successful!");
        const userRole = response.user?.role || "student";
        router.push(getDashboardRoute(userRole));
      } else {
        toast.error(response.message || "Authentication failed");
      }
    } catch (error: any) {
      // Handle 404 - endpoint not implemented yet
      if (error.response?.status === 404) {
        toast.error("Google login endpoint not available. Please use email/password login or contact support.");
        console.error("Backend endpoint /api/v1/auth/google not implemented yet");
        return;
      }
      
      const errorMessage = error.response?.data?.message || error.message || "Authentication failed";
      
      // Check if error is about email domain
      if (errorMessage.includes("navgurukul.org") || errorMessage.includes("email") || errorMessage.includes("domain")) {
        toast.error("Only @navgurukul.org emails are allowed");
      } else if (error.response?.status === 400 || error.response?.status === 401) {
        toast.error(errorMessage);
      } else {
        toast.error("Google authentication failed. Please try again or use email/password login.");
      }
    } finally {
      setIsLoading(false);
    }
  };

  const login = useGoogleLogin({
    onSuccess: handleGoogleSuccess,
    onError: () => {
      toast.error("Google login failed. Please try again.");
      setIsLoading(false);
    },
    flow: "auth-code", // Use authorization code flow for better security
  });

  return (
    <Button
      type="button"
      variant="outline"
      className="w-full h-11 border-2"
      onClick={() => {
        setIsLoading(true);
        login();
      }}
      disabled={isLoading}
    >
      <svg className="mr-2 h-5 w-5" viewBox="0 0 24 24">
        <path
          fill="currentColor"
          d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
        />
        <path
          fill="currentColor"
          d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        />
        <path
          fill="currentColor"
          d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        />
        <path
          fill="currentColor"
          d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        />
      </svg>
      {isLoading
        ? "Connecting..."
        : isSignup
        ? "Sign up with Google"
        : "Continue with Google"}
    </Button>
  );
}

